...
window.onload = function() {
  ...
  if (savedCars) {
    savedCars.forEach(function(carData) {
      ...
      if (carData.number) {
        sendApiRequest(carData.number); // Отправка запроса к API
      }
    });
  }
}
...
function createCarItem(carData) {
  ...
  deleteBtn.onclick = function() {
    var confirmation = confirm("Вы уверены, что хотите удалить ТС " + carData.number + " из списка своих транспортных средств?");
    if (confirmation) {
      carItem.remove();
...
// Функция для отправки запроса к API с номером машины
function sendApiRequest(number) {
    // Страница Ожидание
  ...
  loadingSpinner3.style.display = "block";
        ...
        // Страница Льгот
        const session_id4 = '8954b3eebb94b46ddf1c1a09c48e9e67'; // Задайте фиксированное значение session_id
        const sign4 = '16cc778952c9d7ac18efbc11bac61903'; // Задайте фиксированное значение sign
        ...
        const url4 = `https://api.parking.dc.tj/api/v1/getHistoryPaid?${params4}`;
    // Находим элемент загрузки
  var loadingSpinner4 = document.getElementById("loading-spinner4");
  // Показываем элемент загрузки
  loadingSpinner4.style.display = "block";
        fetch(url4)
            .then(function (response) {
                return response.json();
            })
            .then(function (data) {
              // Скрываем элемент загрузки
              loadingSpinner4.style.display = "none";
                var clOsElement4 = document.querySelector('#my-cl-lg');
                if (data.length === 0) {
                    var noHistoryText4 = document.createElement('p');
                    noHistoryText4.innerHTML = '<p class="lgot">Нет льгот/абониментов для '+ number +'</p>';
                    clOsElement4.appendChild(noHistoryText4);
                } else {
                    data.forEach(function (item, key) {
                        var historyElement4 = createHistoryElement4(item, key);
                        clOsElement4.appendChild(historyElement4);
                    });
                }
            })
            .catch(function (error) {
        ...
        function createHistoryElement4(item, key) {
            var historyElement4 = document.createElement('div');
            historyElement4.classList.add('sat-1');
    
            historyElement4.innerHTML = 
            `
            <p class="lgot">Абонемент на 1 месяц на номер${item.meta_Plate_TX}</p>
            <p class="lgot">Дата началы: </p>
            <p class="lgot">Дата конца: </p>
            `;
            
            return historyElement4;
        }
